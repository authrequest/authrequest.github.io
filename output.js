var _cf = _cf || [],
	bmak = {
		...
		sed                         : function () {
			...
			var r;
			return (
				(r = null != window.document.documentElement.getAttribute('selenium') ? '1' : '0'),
				[ a, t, e, n, o, m, r ].join(',')
			);
		},
		cma                         : function (a, t) {
			try {
				if ((1 == t && bmak.mme_cnt < bmak.mme_cnt_lmt) || (1 != t && bmak.mduce_cnt < bmak.mduce_cnt_lmt)) {
					...
					e && e.pageX && e.pageY
						...
						: e && e.clientX && e.clientY && ((n = Math.floor(e.clientX)), (o = Math.floor(e.clientY)));
					var m = e.toElement;
					null == m && (m = e.target);
					var r = bmak.gf(m),
				...
				if ((1 == t && bmak.pme_cnt < bmak.pme_cnt_lmt) || (1 != t && bmak.pduce_cnt < bmak.pduce_cnt_lmt)) {
					...
					if (n && 'mouse' != n.pointerType) {
						...
						n && n.pageX && n.pageY
							...
							: n && n.clientX && n.clientY && ((o = Math.floor(n.clientX)), (m = Math.floor(n.clientY)));
			...
			try {
					...
					m && n && (n = 0 != m && 0 != n && m != n ? -1 : 0 != n ? n : m),
						0 == i &&
							...
							(n =
								3 == t && n >= 32 && n <= 126
									? -2
									: n >= 33 && n <= 47 ? -3 : n >= 112 && n <= 123 ? -4 : -2),
						l != bmak.prevfid ? ((bmak.fidcnt = 0), (bmak.prevfid = l)) : (bmak.fidcnt = bmak.fidcnt + 1);
				...
				o && e && bmak.ke_cnt++,
					!bmak.js_post ||
						1 != t ||
						(13 != n && 9 != n) ||
						((bmak.aj_type = 3), bmak.bpd(), bmak.pd(!0), (bmak.ce_js_post = 1));
			} catch (a) {}
		},
		cta                         : function (a, t) {
			try {
				...
				1 == t ? bmak.tme_cnt++ : bmak.tduce_cnt++,
					...
					bmak.js_post &&
						...
						((bmak.aj_type = 5), bmak.bpd(), bmak.pd(!0), (bmak.ce_js_post = 1), bmak.aj_indx_tact++);
			} catch (a) {}
		},
		...
		cdoa                        : function (a) {
			try {
				...
				bmak.js_post &&
					...
					((bmak.aj_type = 6), bmak.bpd(), bmak.pd(!0), (bmak.ce_js_post = 1), bmak.aj_indx_doact++),
					bmak.doa_throttle++;
			} catch (a) {}
		},
		cdma                        : function (a) {
			try {
				...
				bmak.js_post &&
					...
					((bmak.aj_type = 7), bmak.bpd(), bmak.pd(!0), (bmak.ce_js_post = 1), bmak.aj_indx_dmact++),
					bmak.dma_throttle++;
			} catch (a) {}
		},
		get_type                    : function (a) {
			return (
				(a = a.toLowerCase()),
				'text' == a || 'search' == a || 'url' == a || 'email' == a || 'tel' == a || 'number' == a
					? 0
					: 'password' == a ? 1 : 2
			);
		},
		...
		cfsubmit                    : function () {
			bmak.sd_debug('<cfsubmit/>'),
				bmak.js_post
					? ((bmak.aj_type = 4), bmak.bpd(), 0 == bmak.ce_js_post && bmak.cns != bmak.ins && bmak.pd(!0))
					: bmak.bpd();
		},
		getdurl                     : function () {
		...
		set_cookie                  : function (a, t) {
			void 0 !== document.cookie &&
				(document.cookie = a + '=' + t + '; path=/; expires=Fri, 01 Feb 2025 08:00:00 GMT;');
		},
		get_cookie                  : function () {
			try {
		...
		bpd                         : function () {
			...
			try {
				...
				try {
					bmak.sd_debug(',s2:' + a),
						(bmak.sensor_data =
							bmak.ver + '-1,2,-94,-100,' + bmak.uar() + '-1,2,-94,-120,' + a.replace(/\"/g, "\\'"));
				} catch (a) {
					bmak.sd_debug(',s3:' + a);
				}
			}
				...
				if (0 == bmak.sdfn.length)
					bmak.sd_debug(',s4'),
						document.getElementById('sensor_data') &&
							(bmak.sd_debug(',s5'), (document.getElementById('sensor_data').value = bmak.sensor_data));
				else {
					bmak.sd_debug(',s6:');
				...
				: void 0 !== document.mozHidden
					...
					: void 0 !== document.msHidden
						...
						: void 0 !== document.webkitHidden &&
							((bmak.hn = 'webkitHidden'), (bmak.vc = 'webkitvisibilitychange')),
				document.addEventListener
					? 'unk' != bmak.hn && document.addEventListener(bmak.vc, bmak.hvc, !0)
					: document.attachEvent && 'unk' != bmak.hn && document.attachEvent(bmak.vc, bmak.hvc),
		...
		encode                      : function (a) {
			...
			return (
				a.length % 3 == 1 &&
					((t = bmak.gb(a, k)), (o = t >> 2), (m = (3 & t) << 4), (b = b + c.charAt(o) + c.charAt(m) + '==')),
				a.length % 3 == 2 &&
					((t = bmak.gb(a, k)),
					(e = bmak.gb(a, k + 1)),
					(o = t >> 2),
		...
		mn_get_new_challenge_params : function (a) {
			...
			if (null != a)
				for (var n = 0; n < a.length; n++) {
					...
					if (o.length > 0) {
						for (
							var m = o[0], r = bmak.mn_abck + bmak.start_ts + o[2], i = o[3], c = o[6], b = 0;
							b < bmak.mn_lcl && 1 == m && bmak.mn_lc[b] != r && bmak.mn_ld[b] != i;
							b++
						);
		...
		mn_h                        : function (a) {
			...
			for (var c = r.length / 4 + 2, b = Math.ceil(c / 16), d = new Array(b), k = 0; k < b; k++) {
				...
				for (var l = 0; l < 16; l++)
					d[k][l] =
						(r.charCodeAt(64 * k + 4 * l) << 24) |
						(r.charCodeAt(64 * k + 4 * l + 1) << 16) |
						(r.charCodeAt(64 * k + 4 * l + 2) << 8) |
						(r.charCodeAt(64 * k + 4 * l + 3) << 0);
			}
		...
		mn_w                        : function () {
			try {
				for (
					var a = 0, t = 0, e = 0, n = '', o = bmak.get_cf_date(), m = bmak.mn_cd + bmak.mn_mc_indx;
					0 == a;
				...
				(bmak.mn_mc_indx += 1),
					bmak.mn_mc_indx < bmak.mn_mc_lmt
						? setTimeout(bmak.mn_w, bmak.mn_tout + e)
						: ((bmak.mn_mc_indx = 0),
							(bmak.mn_state = 0),
							...
							bmak.js_post && ((bmak.aj_type = 8), bmak.bpd(), bmak.pd(!0)));
			} catch (a) {
		...
		listFunctions               : {
			...
			_setAu                : function (a) {
				'string' == typeof a &&
					0 === a.lastIndexOf('/', 0) &&
					(bmak.cf_url =
						(bmak.fsp ? 'https:' : document.location.protocol) + '//' + document.location.hostname + a);
			},
		...
		(a.fpcf = t),
			(t.sf4 = function () {
				...
				return !(
					...
					!(~a.indexOf('iPad;') || ~a.indexOf('iPhone') || ~a.indexOf('Mac OS X 10_5'))
				);
			}),
			(t.fpValstr = '-1'),
			(t.fpValCalculated = !1),
